//   Implementation works fine, but doesn't accurately call yellow variegation in fly butts

/* Note I am currently converting images into Lab colorspace because I think that I have more
power to resolver yellow variegation.  If you want to try different colorspaces you need to change 5 lines of code
where what is called variegated is output and then 4 lines of code where the measurement is made.
*/
...
5. Use PCA or Kmeans to automatically detect RGB values corresponding to variegated (or other colorspace, e.g. HSB, Lab)
...
// Set values using Color inspector 3D, which is a great tool for determining how 
// 		different colors are represented in different color spaces…histogram is useful within that plugin.
// NOTE YOU MAY NEED TO ADJUST VALUES BASED ON YOUR CAMERA SET-UP/GENETIC BACKGROUND (of your flies)
// Special note if you don't use RGB colorspace that Color Threshold convert an RGB image
//		to YUV and CIE Lab colour spaces but with values mapped into an 8-bit space...
//		so you may need to convert your image to a 8-bit Lab stack and then use Color Inspector 3D on that
//		to get the appropriate values for Color Threshold (and for below).  Additionally there seems
//		to be another bug so I recoded the below

Rmin=0; // When using Lab colorspace I used the R variable to signify L, G is a, and B is b.
Rmax=40;
Gmin=0;
Gmax=15;
Bmin=0;
...
/////////////////////////////////
// SAVE TIF OF WHAT GETS COUNTED AS VARIEGATED, note that variegated is (may be) contiguous with the black background //////////////////////////////
 
imgNameDD=getTitle(); // Added by JMS
...
saveAs("Jpeg", dir1+outputPath+imgNameDD+"Lab"+Rmin+"-"+Rmax+"_"+Gmin+"-"+Gmax+"_"+Bmin+"-"+Bmax+"_var-labeled.jpg");
...
for (i=0; i<n; i++) {
	...
	y = getResult('Y');
//
	// BREAK TO CALCULATE AREA WITHIN EYE ABOVE A CERTAIN THRESHOLD --- added by JMS
