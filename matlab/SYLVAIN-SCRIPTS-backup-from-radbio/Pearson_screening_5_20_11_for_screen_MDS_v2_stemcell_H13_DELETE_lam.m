...
target_dir = '/mnt/orona/jswenson/temp-for-matlab/2-3-14_temp/';
output_dir = '/mnt/orona/jswenson/temp-for-matlab/2-3-14_temp-lamin-segment/';
...
blurr_rad = 30; % the larger, the milder the background correction: 30 for the screen, 60 (or 30.  30 when segmenting with Lamin) for the 60X DV, for cytopsin 90, for live HP1a 150
% for live HP1a 60X use 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
search_str = '*';
list_dir = dir([target_dir search_str]);
nuc_chan = 2; %for screen nuc_chan =1
th_val = []; % IF set to [], then system does isodata to segment nucleus. If not, then uses constant threshold.  [] for screen
...
nuc_rad = 25; %use 8 for the screen  %use 20 when using the DV 60X with Kc cells, 25 for S2, for live HP1a 40X use 10, for live HP1a 60X use 20
...
for i_t=1:num_t % Open directory number i_t
    ...
    for i_img = 1:num_img % Read image i_img for time point i_t 
        if strcmp(lower(file_ext), 'tif')
            first_part_file = regexp(list_img(i_img).name, '_w', 'split');   % for new format from screening center - SVC 7/17/2012
        ...
        % file automatically
        if i_img == 1
            file_namej = [name(1:4) '_pearson_summary']; %plate_name, date, pearson_summary_campus_40X
