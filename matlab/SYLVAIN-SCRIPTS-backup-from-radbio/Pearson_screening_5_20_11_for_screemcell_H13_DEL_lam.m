...
target_dir = '/mnt/orona/jswenson/InCell6000/Joel-and-Amy20x--second-plate/';
output_dir = '/mnt/orona/jswenson/InCell6000/Joel-and-Amy20x--second-plate/';
...
nuc_rad = 8; %use 8 for the screen  %use 20 when using the DV 60X with Kc cells, 25 for S2, for live HP1a 40X use 10, for live HP1a 60X use 20
...
for i_t=1:num_t % Open directory number i_t
    ...
    for i_img = 1:num_img % Read image i_img for time point i_t 
        ...
        % file automatically
        if i_img == 1
            file_namej = [name(1:4) '_pearson_summary_new']; %plate_name, date, pearson_summary_campus_40X
            seg_save_dir = [output_dir plate_str '_pearson_nuc_seg_saved_new/'];
            mkdir(seg_save_dir);
