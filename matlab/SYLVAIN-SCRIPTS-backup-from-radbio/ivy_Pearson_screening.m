...
%
%Main directory
target_dir = '/mnt/orona/ivy/10june2011/';
seg_save_dir = ['/data/Collaborations/Karpen/ivy/10june2011/' 'pearson_nuc_seg_saved/'];
mkdir(seg_save_dir);
search_str = '*';
list_dir = dir([target_dir search_str]);
nuc_chan = 3;
th_val = []; % IF set to [], then system does isodata. If not, then uses constant threshold
chan1 = 1;
chan2 = 2;
nuc_rad = 15;
ofp = fopen(['/data/Collaborations/Karpen/ivy/10june2011/' 'pearson_summary_' date '.txt'],'w');
