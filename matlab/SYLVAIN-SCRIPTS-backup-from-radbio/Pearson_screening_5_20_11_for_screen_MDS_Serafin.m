...
%
%Main directory
%target_dir = '/mnt/orona/jswenson/screen_images_from_facility/start_of_screen/1stfourplates_of_screen/Joel-S-110728-40X-plate-kc-131B79/2011-07-29/';
%target_dir = '/mnt/orona/jswenson/screen_images_from_Sylvain_40X/tifs/';
target_dir = '/mnt/orona/SERAFIN/Pictures/080415_H3K56Me3_IR_CIDcontrol/analysis/';
%output_dir = '/data/Collaborations/Karpen/Master_Joel/Joel/Screen/plates_imaged_on_sylvains_scope/';
output_dir = '/mnt/orona/SERAFIN/Pictures/080415_H3K56Me3_IR_CIDcontrol/test_results_546vsCy5/';
...
%%%%%%%%%%%%%%%%%%%%
file_ext = 'dv'; % CHANGE HERE FOR DIFFERENT EXTENSION (e.g 'dv' = .dv, 'tif' = .tif).   If for screening don't forget to change line 40 to list_img = dir([target_dir list_name{i_t} '/*_w1_*.' file_ext]);
...
blurr_rad = 30; % the larger, the milder the background correction.  normally 30.  for cytospin 90. for live HP1a 150
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
search_str = '*';
...
chan1 = 3; %for campus 3 is ph2av, 2 is hp1a....for Sylvain's scope it is the opposite
chan2 = 4;  % To make sure R analysis is labelled properly set chan1 to be pH2Av and chan2 to be HP1a
nuc_rad = 10; %% DV 40X 20 seems to work well with Kc cells.  For live HP1a 40x use 10
...
for i_t=1:num_t % Open directory number i_t
    ...
    for i_img = 1:num_img % Read image i_img for time point i_t 
        ...
        % file automatically
        if i_img == 1
            file_namej = [name plate_str '_' date_str '_pearson_summary_sylvain_40X']; %plate_name, date, pearson_summary_campus_40X
            seg_save_dir = [output_dir plate_str '_pearson_nuc_seg_saved/'];
            mkdir(seg_save_dir);
            ofp = fopen([output_dir file_namej '.txt'],'w');
            fprintf(ofp,'Image\tplate name\twell name\tnuc#\tnuc size\tPearson with chan1\tPearson with chan2\tPearson chan1 & chan2\t');
            ...
            fprintf(ofp,'site name\tdate imaged\n');
        end
        try % some images are not with the same # of channels... Escape and print out there was a Pb with it
            ...
            nuc_ring = round(dt(nuc_mask>0)/4); 
            ms_ring = measure(nuc_mask,nuc_ring,'MaxVal');
            ring_bg = msr2obj(nuc_mask,ms_ring,'MaxVal');
            nuc_ring = ring_bg-nuc_ring;
            nucr1 = and(nuc_mask,nuc_ring<2);
            nucr2 = nuc_ring==2;
            nucr3 = nuc_ring==3;
            nucr4 = nuc_ring>3;
