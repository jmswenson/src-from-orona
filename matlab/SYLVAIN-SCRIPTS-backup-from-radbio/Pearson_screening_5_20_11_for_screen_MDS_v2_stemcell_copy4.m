...
%Main directory
%target_dir = '/data/Collaborations/Karpen/Master_Joel/384_stem_cell_center_test_images/384-test-120716-plate-135b72/';
%output_dir = '/data/Collaborations/Karpen/Master_Joel/384_stem_cell_center_test_output/';

%%%%%   DAPI is chan1, CID is chan2, H3K9me2 is chan3, HP1a is chan4
%%%%%   ....this is the order they were imaged in
target_dir = '/mnt/orona/ShareWithGenuser/Joel1/assay3/';
output_dir = '/mnt/orona/ShareWithGenuser/Joel1_outp/';
%  NOTE: remove all "thumbnail" images from directory before running analysis
...
chan1 = 3; %for campus 3 is ph2av, 2 is hp1a....for Sylvain's scope it is the opposite: screen chan1=2, chan2=3
...
for i_t=1:num_t % Open directory number i_t
    if strcmp(file_ext, 'tif')
        list_img = dir([target_dir list_name{i_t} '/*_w1*.' file_ext]);  % This (with '/*_w1_*.' will only work on images from the screening center or if I have '_w1_' in the name: noted by Joel
        if isempty(list_img)
            list_img = dir([target_dir list_name{i_t} '/*_w1*.' upper(file_ext)]);  % This (with '/*_w1_*.' will only work on images from the screening center or if I have '_w1_' in the name: noted by Joel
        end
    else
    ...
    for i_img = 1:num_img % Read image i_img for time point i_t 
        if strcmp(lower(file_ext), 'tif')
            first_part_file = regexp(list_img(i_img).name, '_w[1-4]', 'split');   % for new format from screening center - SVC 7/17/2012
        ...
        plate_i = findstr(name,'-screen-')+15;
        plate_str = name(plate_i:und_i(1)-4);
