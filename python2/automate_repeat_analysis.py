...
for x,i in enumerate(aa):  ## x is the counter, i is the item being iterated thru (name of a file)
	...
	if i.count('bz2'): # if files contains bz2 then do what is in this if statement
		print i
		cmd = [program, '-dk', full_name]
		uzip = SP.Popen(cmd)
		uzip.wait() # Allows the program to finish
		fastq_fn = full_name[:-4]
		rpt_outp = full_name[:-10] + '.top_repeats_plus'
		...
		aa = btierun.communicate() #this returns a tuple
		loc_rp = list(aa)[1].index('reads process')  #this converts the tuple to a list, looks at the first element and then returns the index in that element that contains "reads process"
		ndx2 = list(aa)[1].index('reads with at least one reported alignment')
		rs_proc = list(aa)[1][loc_rp:ndx2-4]  ## saves the 'reads processed' to a variable
						      #btieout = btierun.stdout.readlines()
		cmd2 = [prog2, grep_loc, rpt_outp]
		greprun = SP.Popen(cmd2)
		greprun.wait()
		print rs_proc
		print
		print
